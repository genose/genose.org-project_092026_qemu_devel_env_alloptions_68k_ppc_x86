...
Une fois le type de processeur stocké en mémoire de manière sécurisée (par exemple dans le registre global %d7 ou R7), le bootloader doit charger l'image de Mac OS Classic présente sur ton disque virtuel immatriculé dans vm_assist.sh :Initialisation du gestionnaire de trappes spécifique : Ton code applique les règles de formatage de pile découverts précédemment (les fameux Stack Frames de Type 7 pour le 68040).Copie de la table des vecteurs système : Tu recopies les adresses physiques de la ROM Mac (fournie par -bios quadra800.rom) vers les vecteurs mémoire, mais en injectant tes sauts MacsBug au milieu de la chaîne de lecture.Saut au Jump-Start de la ROM : Tu passes le contrôle à l'adresse de boot de la ROM d'Apple (0x40800010 généralement sur les Mac 32-bit Clean), qui va initialiser le Finder de Mac OS Classic.
